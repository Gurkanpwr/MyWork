...
void SinWaveGen(){
	for (int i=0;i<=SAMPLES;i++){
		sine_value[i]=((sin(i*2*PI/1024)+1)*(65535/2));
		//HAL_Delay(20);}
	}
}
...
int main(void)
{
  ...
  MX_TIM2_Init();
  /* USER CODE BEGIN 2 */
 // HAL_DAC_Start_DMA(&hdac, DAC_CHANNEL_2,sine_value, 1024, DAC_ALIGN_12B_R);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  if(htim == &htim2){
	  ...
	  HAL_DAC_SetValue(&hdac, DAC1_CHANNEL_2, DAC_ALIGN_12B_L, sine_value[i]);
  }
}
/* USER CODE END 4 */
